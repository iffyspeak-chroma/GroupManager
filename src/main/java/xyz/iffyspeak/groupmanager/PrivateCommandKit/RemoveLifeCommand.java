...
public class RemoveLifeCommand extends BasicCommand {
    public RemoveLifeCommand() {
        super(
                "remlife",
                "<green>Remove a life (or multiple) from a player</green>",
                "<red>remlife (username)</red><yellow> (amount)</yellow>",
                true);
    }
    ...
    public void execute(List<String> args, Player sender) {
        ...
        if (args.size() >= 2)
        {
            ...
            for (Player p : Bukkit.getOnlinePlayers())
            {
                ...
                {
                    String retstr = "<color:#00ff00>Removed <yellow>" + amount + "</yellow> lives from<yellow> " + p.getName() + "</yellow>.</color>";
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (args.get(0).equals(p.getName())) {
                String retstr = "<color:#00ff00>Removed <yellow>1</yellow> life from<yellow> " + p.getName() + "</yellow>.</color>";
                int curLives = SQLToolkit.getPlayerLives(Globals.Database.mySQL, String.valueOf(p.getUniqueId()));
